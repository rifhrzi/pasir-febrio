...
export default function Expense() {
  ...
  const handleExport = async () => {
    try {
      ...
      link.href = url;
      link.setAttribute('download', `expense_export_${Date.now()}.xlsx`);
  ...
  return (
    <Layout>
      <div className="space-y-8">
        <section className="rounded-3xl bg-gradient-to-r from-rose-600 via-pink-600 to-red-600 p-8 text-white shadow-2xl">
          <p className="text-xs uppercase tracking-[0.35em] text-white/70">Expense Management</p>
          <h1 className="mt-3 text-4xl font-semibold">Expense</h1>
          <p className="mt-4 max-w-2xl text-sm text-white/80">
            Catat pengeluaran dengan rapi menggunakan preset kategori sesuai catatan operasional.
          </p>
        </section>
        ...
        <section className="rounded-3xl border border-slate-200 bg-white/90 p-6 shadow-xl ring-1 ring-black/5 backdrop-blur">
          <div className="flex flex-wrap items-start justify-between gap-4 border-b border-slate-100 pb-4">
            <div>
              <h2 className="text-2xl font-semibold text-slate-900">Catat expense dengan rapi</h2>
          ...
          <div className="grid gap-4 py-6 sm:grid-cols-2">
            ...
            <div className="rounded-2xl border border-slate-100 bg-white p-4 shadow-inner">
              ...
              <p className="mt-2 text-2xl font-semibold text-rose-600">{formatCurrency(totalAmount)}</p>
            </div>
          </div>
